...
def coletar_bcb_sgs(codigo_serie: int, nome: str):
    url = f"https://api.bcb.gov.br/dados/serie/bcdata.sgs.{codigo_serie}/dados/ultimos/30?formato=json"
    resp = requests.get(url, timeout=30)
    resp.raise_for_status()
    dados = resp.json()
    hoje = date.today()
    caminho = (f"/Volumes/fininsight_dev/landing/raw/bcb/{nome}/"f"ano={hoje.year}/mes={hoje.month:02d}/dia={hoje.day:02d}/dados.json")
